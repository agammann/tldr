...
The local MCP was registered and enabled in Codex. An SDK client launched the exact command read back from that configuration, discovered all five tools, reviewed the fictional terms and verified that the browser bridge started and created its pairing file. This confirms the configured command works. It does not claim that this conversation invoked a newly registered tool or that the extension was installed in the user's everyday Chrome profile. That profile was unavailable to the browser automation interface, so normal Chrome installation and the first assistant review remain user setup steps.
